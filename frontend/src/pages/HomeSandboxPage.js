...
const HomeSandboxPage = ({ hideRotasProgramadas = false, hidePainelViagens = false, hideFiltros = false, headerFirst = false }) => {
  ...
  // Labels for the counter cards (weekday/month short in pt-BR)
  const weekdayLabel = useMemo(() => {
    const s = new Date().toLocaleDateString('pt-BR', { weekday: 'short' });
    const withDot = s.endsWith('.') ? s : s + '.';
    return withDot.charAt(0).toUpperCase() + withDot.slice(1);
  }, []);
  const monthLabel = useMemo(() => {
    const s = new Date().toLocaleDateString('pt-BR', { month: 'short' });
    return s.endsWith('.') ? s : s + '.';
  }, []);

  // Helpers: normalize text and parse date safely (avoid TZ shifts)
  ...
  // Try multiple fields that can represent the conclusion/return date
  ...
  // Strict conclusion date for month counter: only definitive fields
  const getMonthConclusionDateStrict = (t) => {
    const candidates = [t?.data_retorno, t?.data_chegada];
    for (const c of candidates) {
      const d = parseToLocalDate(c);
      if (d) return d;
    }
  ...
  // Only consider DB column status_viagem equal to 'Concluida'
  const isStatusConcluida = (value) => normalizeText(value) === 'concluida';

  // Count trips concluded in the current month
  const tripsCompletedThisMonthCount = useMemo(() => {
    const now = new Date();
    const y = now.getFullYear();
    const m = now.getMonth(); // 0-based
    return (trips || []).filter((t) => {
      const status = t?.status_viagem;
      if (!isStatusConcluida(status)) return false;
  ...
  // Count trips concluded in the current year
  ...
  const tripsActiveTodayCount = useMemo(() => {
    const isActiveOn = (trip, dateKey) => {
      const startStr = toDateKey(trip?.data_saida);
      const endStr = toDateKey(trip?.data_retorno_prevista || trip?.data_saida);
      ...
      if (isNaN(end.getTime())) return target.getTime() === start.getTime();
      start.setHours(0,0,0,0);
      end.setHours(0,0,0,0);
      target.setHours(0,0,0,0);
      return target >= start && target <= end;
    };
    return (trips || []).filter((t) => isActiveOn(t, todayKey)).length;
  }, [trips, todayKey]);

  const rotasAndamentoCount = useMemo(() => {
