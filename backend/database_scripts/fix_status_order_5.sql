...
BEGIN
    ...
    -- 3. Criar novo ENUM temporário com todos os status
    ...
    -- 8. Atualizar a tabela original usando a nova tabela temporária
    UPDATE viagens v
    SET status_viagem = t.status_viagem
    FROM temp_viagens_new t
    WHERE v.viagemid = t.viagemid;

    -- 9. Limpar as tabelas temporárias
    DROP TABLE temp_viagens;
    DROP TABLE temp_viagens_new;

    -- 10. Remover o ENUM antigo
    DROP TYPE status_viagem;

    -- 11. Renomear o novo ENUM
    ALTER TYPE status_order_temp RENAME TO status_viagem;

    -- 12. Restaurar o valor padrão
